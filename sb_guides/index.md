...
1. "Do or Die" countdown - When you enter a room, all exits will be 
...
To begin with, collect 100 red orbs on the horserider's spear tip. Jump onto the horse from the stairs first of all, and then hop-jump again onto its neck (if the first jump didn't get you there). From the neck, there are two ways of getting on top of the spear. The first way is to stand next to the spear, do a full jump straight up, then press the jump button to do a kick-jump onto the tip. The second way is to hold down the jump button (you can't stop Dante from doing an initial jump first), and then walk around the base of the neck with the jump button held down - there is a trigger point which, if you pass over it, will make Dante automatically jump to grab the tip and haul himself onto it. You don't want to be messing around for 5 minutes trying to land on the spear tip, so practice for a while and find the method which you can do fastest.
...
To kill a Fetish stylishly with the Force Edge, use this combo string: 
 - grounded High Time > Multi-Stab Combo with High Time finisher (x2) > finish off with anything you like

Launch the Fetish with a High Time, get behind it (so it can't parry any of your hits), do a Multi-Stab Combo with HT finisher twice in a row (after each HT, get behind the Fetish again if need be), and then finish the Fetish off with a 4-Hit combo or whatever. You really do get a lot more red orbs from killing a Fetish with a Stylish combo rating than a dull one, so it's definitely worth doing.

Once you've killed 3-4 Fetishes, tear down the stairs back to the main hall and use your new found rusted key to gain entrance to the biplane room. There is a hidden red orb cache on the biplane nose worth 40 red orbs. It's possible to do a tricky kick-jump directly onto the nose, but meh: I usually botch this up >_>. A much easier way is to kick-jump onto the left wing, stand above the nose, and hold down the jump button to make Dante do a gentle hop down onto it. After obtaining the orb cache, go to the dial and light it up: a fast 4-Hit combo with a rising High Time > downward slash finisher is *just* sufficient to activate it.
...
The single fastest way to defeat the first wave is to use sword-cancelled 2-hit combos - that is, pairs of slashes, with the second slash of each pair being cancelled. I have to say, *now* is not a very good time to scratch your head and exclaim, "Dur, what's sword-cancelling??", but if you do, pause your game, go to the Advanced Techniques section and read about it there. In the meantime, don't fret, since - luckily - sword-cancelling isn't vital for this part.

If you don't know how to sword-cancel, ordinary combos will suffice. Use 4-Hit combos with HT finishers. The key to a fast victory is to combo *multiple* enemies whenever you can - use a grounded HT to launch 2 or more enemies and combo away.

There is even a *third* method: E&I spam. Just make enemies drop down and fire as fast as possible while moving backwards.
